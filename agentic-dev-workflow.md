...
- **Standing rules** (project instructions, re-injected per turn — see Automation): divergence reporting — *if the spec conflicts with observed reality, stop and report; never work around it* (silent workarounds encode the contradiction into the code); pushback — *refuse to implement anything violating DECISIONS/SPEC without flagging it*; journal-append on any non-obvious choice or dead end.
...
Claude Code exposes a fixed set of hook events; the API moves, so the bindings below are a sketch to confirm against the current docs before wiring. The load-bearing observation is that different events decay differently: a `SessionStart` injection is fresh at turn one and progressively buried as context fills, so anything that must stay live belongs on a per-turn event instead.

**Claude Code hooks** (sketch):
- `SessionStart` — inject SPEC, DECISIONS, LESSONS, ASSUMPTIONS into context; emit the session probe as the scripted opener. This is one-shot context loading; it does not keep anything live as the session runs.
- `UserPromptSubmit` — fires on every user turn before the model sees the prompt, and can inject context. This is the right home for the standing rules (divergence reporting, pushback, journal-append): re-asserting them every turn counters the decay that buries a once-per-session injection. The judgment-based rules are exactly the ones this workflow says must be kept few, short, and re-injected — per-turn fits that better than per-session.
...
- `SubagentStop` — the same distillation gate for fresh-instance subagents. The assumption audit (§2) and spikes (§3) run as fresh instances; if those are subagents, this hook forces them to end with an explicit verdict — assumptions enumerated, or validated/falsified with evidence — the way `Stop` gates the main session. Without it, only the main agent's exit is gated and the audit/spike outputs ride on instruction-only discipline.
...
- `PreCompact` (optional) — re-read the four artifacts into the post-compaction window. The artifacts are the durable memory precisely because context is lossy; this keeps a compaction from silently dropping the spec mid-session.

The remaining events (`PostToolUse`, `Notification`, `SessionEnd`) are deliberately unused at first. `PostToolUse` could prompt for a journal entry the moment a non-obvious edit is committed, but mechanical per-edit prompting is the kind of forced-completion pressure that poisons the journal with filler — add it only if a concrete need appears. The hooks and skills here package naturally as a single Claude Code **plugin**, which is what makes them fire automatically rather than depending on a human remembering to install them — the whole point of moving discipline from memory into structure.
