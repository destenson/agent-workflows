...
| `gstreamer-rs-workflow` | Extends the embedded loop for GStreamer/DeepStream video pipelines with a forked `gstreamer-rs` element (`rtspsrc2`) on Jetson. Ships the failure-triage, pipeline-reduction, lifetime-review, fork-triage, and reconnect-repro skills, the `PIPELINE.md` and harness/fault-server scaffolders, and an optional env-gated reminder that surfaces the lifetime checklist when editing the forked element. |
...
| `prototype-to-product` | Hardens a working prototype into a pre-release product. Keeps a `HARDENING.md` gap ledger — the release bar plus the itemized gaps between the prototype and that bar — loaded at session start together with the hardening standing rules. Ships assess-prototype (build the ledger), define-release-target (set the bar and prioritize against it, extending `SPEC.md` if present), and convert-prototype (work the ledger down one item per commit). Composes with `agentic-workflow`; independent of it. |
