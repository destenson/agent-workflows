---
name: fleet-status
description: Gather a read-only, point-in-time state snapshot across the fleet from FLEET.md and report what is healthy, degraded, or unreachable. Use when the user asks for fleet status, a health snapshot, or to check how the systems are doing.
---
...
Produce a current-state snapshot of the fleet. This is read-only: run only commands that report state, never any that change it. If the user named a host or group, scope to that; otherwise cover every host in FLEET.md.
...
This skill only observes and reports. If it surfaces something broken, hand off to the troubleshooting-loop skill to work it — do not start changing state from inside a status sweep.
