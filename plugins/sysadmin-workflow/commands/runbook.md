---
description: List the runbooks in RUNBOOKS.md, or execute a named one step-by-step with each state-changing step narrated and verified.
argument-hint: "[runbook name to run — empty to list all]"
---
...
**No argument** — list every runbook in RUNBOOKS.md: name and its "when to use" line, so the user can pick. Do not run anything.

**A runbook name in $ARGUMENTS** — locate it in RUNBOOKS.md. If no exact match, list the closest names and stop; do not run a runbook the user did not ask for.
...
5. If a step fails partway, follow the runbook's **rollback**. Record what happened in CHANGELOG.md (and open an incident with /incident if it left the host in a bad state).
