---
description: Add a system to the fleet — either onboard an existing host, or provision a new one (e.g. a Docker container) and then onboard it — recording it in FLEET.md.
argument-hint: "[name] [existing|new]"
---
...
Determine the mode from $ARGUMENTS, or ask the user if it is not clear:
...
1. **Determine the provisioning method — do not guess it.** Take it from $ARGUMENTS, or from the conventions section of FLEET.md, or ask the user. The method may be a Docker container, a `docker compose` service, a cloud instance via the relevant CLI, a VM, or anything else; this command is tool-agnostic and does not assume Docker.
