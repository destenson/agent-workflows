---
description: Open a new incident in INCIDENTS.md (or update an open one) with the full structure, then work it through the troubleshooting loop.
argument-hint: "[short symptom title | the title of an already-open incident]"
---
...
Read INCIDENTS.md and decide which case applies:

**Continuing an open incident** — if $ARGUMENTS matches an existing entry whose status is not `resolved`: load it, summarize its current state and what has been ruled in/out so far, and pick up the investigation from there. Do not open a duplicate.
...
If $ARGUMENTS is empty, ask the user for the symptom and which host(s) it affects before opening anything.
